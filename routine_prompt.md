...
### 3. データファイルの作成
リポジトリ直下の `build_pdf.py` が読み込む `data.json` を、`schema.md`(同リポジトリ内)またはbuild_pdf.py冒頭のdocstringに記載のスキーマに従って作成する。exec_summary(その日の市場全体の要約)、fund_lag_note(基準価額タイムラグに関する注意書き。鮮度チェックで問題が見つかった場合はその旨も含める)、holdings(保有銘柄分)、news(5件)、outlook(今後の動向予測。short_term(短期・1週間)、medium_term(中期・1〜2か月)、long_term(長期・1年)の3つの観点で、当日の市況・ニュースを踏まえた予測コメントを記載する)を全て埋めること。
