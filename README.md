...
- mainブランチへのPull Request作成・マージと、Zapier(Gmail)経由でのメール送信まで自動で行う設計です。
...
- **実行日の判定はセッションのシステムコンテキストの `currentDate` を必ず使うこと。** `data/`・`reports/`にある最新の既存ファイルの日付を「今日」だと推測して当日分の実行をスキップしてしまう事故が過去に発生しています(詳細は `CLAUDE.md` 参照)。
